...
static void Mutex_lock(mutex_t* mutex)
{
  uint32_t cur = cmpxchg(&mutex->futex_word, 0, set_lock(1)); 
    ...
    for (;;) { 
      while (!is_locked(cur)) { 
        uint32_t prev = cmpxchg(&mutex->futex_word, cur, set_lock(cur)); 
        if (prev == cur) return; 
        for (uint32_t i = 0;
             i < BUSYWAIT && is_locked(cur);
             i++) {  
          cur = load(&mutex->futex_word, seq_cst);
        }
      }
      while (is_locked(cur)) { /*@\label{line:gustedt1:subloop2}@*/
        futex_wait(&mutex->futex_word, cur); /*@\label{line:gustedt1:futexwait}@*/
        cur = load(&mutex->futex_word, seq_cst);
      }
    }
}
...
{
  uint32_t prev = fetch_sub(&mutex->futex_word, set_lock(1), seq_cst); /*@\label{line:gustedt1:decrement}@*/
    if (prev != set_lock(1)) {
      futex_wake(&mutex->futex_word, 1); /*@\label{line:gustedt1:futexwake}@*/
    }
}
